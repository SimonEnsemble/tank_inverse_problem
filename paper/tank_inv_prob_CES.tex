...
A key advantage of MCMC is that it avoids the need to compute the evidence---a high-dimensional integral of the prior multiplied by the likelihood function \cite{van2021bayesian}.
...
Each iteration involves evaluating the forward model under multiple sets of inputs/parameters (for the likelihood and its gradient) \cite{paun2022emulation,patel2024multi}. 
% TODO: check
...
(When constructing our likelihood, we omit the last three data points because surface tension stopped flow out of the orifice despite $h>h_o$---a surface tension phenomenon not accounted for in the model.)

...
% Research highlights
\begin{highlights}
\item We integrate experimental data, mathematical modeling, and Bayesian statistical inversion to solve an inverse problem with quantified uncertainty.
\item The objective is to reconstruct the horizontal cross-sectional area profile of an unseen solid contained in a tank.
\item The data for the inverse problem are time series measurements of the liquid level in the tank as it drains of liquid, driven by gravity, through a small hole.
\item As the liquid level drops, it effectively ``scans'' or ``images'' the area profile of the solid, which displaces water in the tank.
...
\subsection{Remarks}
For the classroom, our experiment, problem setup, and analysis can be simplified by 
(i) using a cylindrical tank, whose area profile is constant and easy to measure,
(ii) drilling a hole in the very bottom of the tank, and
(iii) recording the liquid level in the tank at various time points by real-time visual inspection or by manual or automatic post-hoc analysis of video taken from a smart phone \cite{brown2009innovative,klein2015video}.
We elected to 
(i) use a tank of a more complicated geometry for a challenge (and, based on the inventory of containers in our kitchens);
(ii) disallow the liquid to 'scan' the bottom of $\approx$1\,cm portion of the solid to illustrate extrapolation via a smoothing prior;
and
(iii) use an immersive liquid level strip to measure the liquid level, corresponding to the practical situation where we cannot see through the opaque tank.
...
(b) larger orifices, so the liquid at the top does not remain flat (necessitating computational fluid dynamics simulations \cite{mathew2014numerical,sakri2017numerical});

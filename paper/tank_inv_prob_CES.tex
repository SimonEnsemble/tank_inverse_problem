...
\title [mode = title]{
Scanning the cross-sectional area of a solid inside an opaque tank 
...
(ii) accounts for (a) the vena contracta of the liquid jet (the cross-sectional area of the liquid jet issuing from the orifice is smaller than that of the orifice, owing to fluid streamlines in the tank, near the orifice, non-perpendicular to the face of the orifice \cite{horsch2020simple}), (b) frictional losses across the orifice, and (c) non-uniformity of the velocity profile; and
...
For a multi-view drawing of the tank geometry with its dimensions marked, see Fig.~\ref{fig:tank_geometry}.
The height of the tank is $h_{\text{max}}$ [cm].
Each cross-section of the tank [parallel to the ground] is a rounded rectangle \cite{rounded_rect} with length $l$ [cm], width $w$ [cm], and corner circle radius $r$ [cm].
...
Now, the cross-sectional area [parallel to the ground] of the tank as a function of height $h$ [cm] from its bottom base is (see Fig.~\ref{fig:rounded_rectangle} for a visual proof):
\begin{equation}
	a(h) = l(h) w(h) + 2 r(h) [l(h) + w(h)] + \pi [r(h)]^2, \label{eq:a_of_h}
\end{equation}
with the rounded rectangle's length, width, and corner-circle radius at height $h$:
\begin{align}
...
\end{align}
with $l_b$, $w_b$, and $r_b$ [cm] and $l_t$, $w_t$, and $r_t$ [cm] characterizing the rounded rectangle forming the bottom and top, respectively, base of the tank.
...
(3) at time $t=0$ [s], allowing the water to drain out (driven by gravity) through the orifice, exiting to the atmosphere; and 
...
(More on the liquid level sensor:)
...
Torricelli's law follows from applying Bernoulli's equation \cite{welty2020fundamentals}, a mechanical energy balance, to a streamline of the flow of water from (1) a point at the top in the body of water in the tank to (2) a point in the outlet stream just outside of the orifice. 
This approximates the water as ideal (incompressible and inviscid) and the flow as at steady-state. See Sec.~\ref{sec:Torricelli} for details.

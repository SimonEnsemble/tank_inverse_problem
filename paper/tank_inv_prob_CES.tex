...
\title [mode = title]{
Scanning the cross-sectional area of a solid inside an opaque, draining tank 
  via inference from liquid level dynamics
 }  
...
In engineering and applied sciences, we often encounter an open-top tank from which liquid drains through a small orifice under gravity-driven flow, exiting to the atmosphere (hereafter, a \emph{draining tank}).
...
(Such a clepsydra may be obtained via a solid of revolution about the vertical axis such that the radius increases with the quartic root of the height. \cite{mills1982newton,d2021torricelli})
\end{mytcbox}
...
the velocity $v$ at which liquid flows out of the orifice is proportional to the square root of the height of liquid above the orifice, $z$, i.e. $v\propto \sqrt{z}$ \cite{mills1982newton}.
% TODO: check that he didn't know g!
Today, we recover Torricelli's observation by applying Bernoulli's (1700–1782) equation \cite{welty2020fundamentals} to a streamline of the flow of liquid from (1) a point just below the top of the liquid inside the tank to (2) a point in the outlet stream just outside of the orifice. 
...
(i) is defined as the ratio of the observed outlet volumetric flow rate to that predicted by Torricelli's law, plug flow for the jet, and the area of the orifice \cite{hicks2014determining};
(ii) accounts for 
   ...
   (b) frictional losses, and (c) possibly, turbulence; and
(iii) depends on the properties of the liquid (e.g., viscosity and density), the geometry of the orifice, and, for laminar flow, the Reynolds number \cite{teoman2022discharge}. 
...
Herein, we employ Bayesian statistical inversion \cite{calvetti2018inverse,waqar2023tutorial,kaipio2006statistical,dashti2013bayesian} to solve, with quantified uncertainty, an inverse problem of reconstruction (or, ``seeing the unseen'' \cite{uhlmann2014inverse}): infer the shape of an exogenous, heavy solid (of unknown geometry but incapable of holding water) contained in a draining tank (of known geometry) from measurements of the liquid level in the draining tank over time.
...
Second, we conduct a tank drainage experiment with an exogenous solid in the tank (a bottle). We then leverage this water level time series data and our calibrated forward and measurement models to obtain a posterior distribution over the area of the solid as a function of height.
Comparing with our [more] direct (via a tape measure) measurements of the solid's geometry, the inferred area of the solid traces its shape reasonably well ($<$10\% mean reconstruction error on the bottle's radius). Our approach may be practically useful to non-destructively determine the shape of an unknown solid, or the porosity, shape, and/or height of packed solid particles, inside of an opaque or underground tank.
...
Each horizontal [parallel to the ground] cross-section of the tank is a rounded rectangle with length $l$ [cm], width $w$ [cm], and corner-circle radius $r$ [cm].
...
The length, width, and corner-circle radius of the rounded rectangle constituting the horizontal cross-section at height $h$ are:
\begin{align}
...
The area of a horizontal cross section of the tank at height $h$ [cm] from its bottom base is (see Fig.~\ref{fig:rounded_rectangle} for a visual proof):
\begin{equation}
...
To map the reading from the level sensor [0-1023 integer values] to the liquid level in the tank [cm], we constructed a calibration curve constituting a smooth 1D spline fit to the observed sensor reading at increments of 1 cm height.
...
To derive a model, we apply a mass balance on the draining tank as the control volume.
Treating the water as incompressible (i.e., constant density), a mass balance implies a volume balance.
So, the rate of decrease of the volume of water in the tank [cm$^3$/s] equals the volumetric outflow rate at each time. We apply simple geometry and Torricelli's law, respectively, to write both terms, then equate them to arrive at our dynamic model for $h(t)$.
...
This approximates the water as ideal (incompressible and inviscid) and the flow along the streamline as at steady-state. See Sec.~\ref{sec:Torricelli} for details.
Torricelli's law matches 
(i) the gain in kinetic energy $(\Delta m) v^2/2$ with the 
(ii) loss in gravitational potential energy $(\Delta m)g(h-h_o)$
when a small parcel of water, of mass $\Delta m$, travels from the top of the liquid in the tank to just outside of the orifice.
\cite{groetsch1993inverse_tl,driver1998torricelli,williams2021vessel}
...
\end{equation}
The integrand is the cross-sectional area of water in the tank at height $y$, which is the area of the horizontal cross-section of the tank $a(y)$ minus that of the exogenous solid $\alpha(y)$ displacing water. (We neglect the small volume of water displaced by the level sensor.) 
...
The \emph{model parameters} $\boldsymbol \theta \in \mathbb{R}^6$ characterize the geometry of the tank and orifice and, loosely, the viscosity and density of water (embedded in $c$):
\begin{equation}
...
\end{equation} We are explicit that $H_{\text{obs}}$ is conditioned upon the inputs, parameters, and noise variance and neglect model discrepancy \cite{brynjarsdottir2014learning}.
...
The likelihood function (i) scores the consistency between (a) the model predictions, with proposed values for the inputs and parameters, of the water level over time and (b) the water level time series data and, hence, (ii) quantifies the support the data lend for each possible set of values for the inputs and parameters \cite{van2021bayesian}. 
...
To approximate the posterior, we conduct a Markov chain Monte Carlo (MCMC) simulation \cite{robert1999monte,van2021bayesian} that sequentially draws a sample of inputs/parameters $(h_0, \boldsymbol \alpha, \boldsymbol \theta, \sigma^2 )$ from the posterior density $\pi_{\text{post}}(h_0, \boldsymbol \alpha, \boldsymbol \theta, \sigma^2 \mid \thedatanomath)$. 
From many samples from the posterior, we draw empirical posterior distributions, compute credible intervals, and plot samples of liquid level trajectories when paired with the forward model.
...
\begin{figure*}[!ht]
    ...
    \end{subfigure}
     ...
     \center
    	\includegraphics[width=0.65\textwidth]{posterior_train_theta.pdf}
...
While our methodology requires access to the liquid level in the tank as it drains, we could infer the liquid level from the volumetric flow rate or the geometry of the liquid jet exiting from the tank \cite{groetsch1999inverse}.

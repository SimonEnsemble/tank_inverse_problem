...
Herein, we leverage this forward model to tackle an inverse problem of reconstruction: infer the shape of an exogenous, heavy solid inside of a tank from measurements of the liquid level in the tank as it drains over time. 
(Because the solid displaces liquid, the rate of decrease of the liquid level provides information about the cross-sectional area of the object at that height; 
...
In our experimental setup, an open-top tank
...
First, we calibrate and test a forward model of the water level dynamics using data from two drainage experiments with a tank devoid of an exogenous solid. 
Second, we conduct a drainage experiment with a tank containing an exogenous solid, then leverage the calibrated forward model and data to infer (i.e., obtain a posterior distribution for) the shape of the solid inside the tank with quantified uncertainty.
Our approach may be practically useful to infer the shape of an unknown solid, or the porosity of packed solid particles, inside of an opaque tank that gives access to liquid level measurements. 
\newline
...
As the tank drains, the liquid ``scans'' the object's cross-sectional area as a function of its height.)
...
Second, we conduct a tank drainage experiment with an exogenous solid in the tank. We then leverage this water level time series data and our calibrated forward model to obtain a posterior distribution over the area of the solid in the tank as a function of height.
Comparing with our [more] direct (via a tape measure) measurements of the solid's area as a function of height, the inferred area exhibits a mean residual of xx and falls within the credible interval xx.
{\color{red} FINISH with results} 
%
...
\label{sec:phaseI}
Our objective is to calibrate then test our forward and measurement models.
The model parameters $\boldsymbol \Theta$ and measurement noise variance $\Sigma^2$ are highly uncertain at this point. 
...
The \emph{calibrated model} constitutes eqns.~\ref{eq:forward_model} and \ref{eq:H_obs_distn} with the resulting posterior distribution of $\boldsymbol \Theta$ and $\Sigma^2$. We test the calibrated model by judging its capability to predict the liquid level trajectory in a replicate tank drainage experiment.
...
Our estimates for the area of the rounded rectangle forming the top and bottom of the tank are $a_{t, \text{obs}}=129.9$\,cm and $a_{b, \text{obs}}=103.0$\,cm. Our measured tank height is $h_{\text{max, obs}}=28.6$\,cm.
%We impose the prior distribution, 
Informed by our [noisy and imprecise] measurements, we impose Gaussian prior distributions:
\begin{align}
...
\end{equation} where $\sigma^2$ is the uncertain variance of the noise from the level sensor in our measurement model.
...
The posterior distribution $\pi_{\text{post}}(h_0, \boldsymbol \alpha, \boldsymbol \theta, \sigma^2 \mid \thedatanomath)$ follows from our prior and likelihood function we construct from the data \thedata (see eqn.~\ref{eq:post}). 
We employ NUTS to obtain samples \thesamples from the posterior distribution ($N_s=750$ over three independent chains). 
...
\paragraph{Posterior predictive check.} Fig.~\ref{fig:posterior_train} shows water level trajectories \themodel with $(\thevars)$ sampled from the posterior distribution, in . 
The posterior trajectories agree reasonably with the measured water levels over time (mean absolute residual: 0.26\,cm). 
Comparing the prior and posterior distributions of water level trajectories in Fig.~\ref{fig:prior_train} and Fig.~\ref{fig:posterior_train}, the much lower variance in the posterior indicates the large amount of information the water level time series data provided about the parameter vector and initial water level.
...
\begin{figure}[!ht]
    ...
    \caption{
      ...
      (b) Samples of water level trajectories \themodel from the prior density $\pi_{\text{pr}}(\thevars)$.
      (c) Time series data \thedata from the tank drainage experiment and samples of water level trajectories \themodel from the posterior $\pi_{\text{post}}(\thevars \mid \thedatanomath)$.
      (d) Visualizing the posterior $\pi_{\text{post}}(\thevars \mid \thedatanomath)$. Top: marginals. Bottom: covariance matrix.      
      }
\end{figure}
...
We approximate this posterior distribution as a multi-variate Gaussian distribution
\begin{equation}
...
Fig.~\ref{fig:test} compares the water level time series data collected from the replicate experiment, \thedata, with the trajectories \themodel predicted by the calibrated model. The predictions by the calibrated model agree well with the experimental data (mean absolute residual: 0.28\,cm). 
...
Our objective is to obtain the posterior distribution of $\mathbf{A}$ characterizing the cross-sectional area of the object as a function of height, $\alpha(h; \mathbf{A})$.

\subsection{Experimental setup}
We set up a tank drainage experiment by:
(i) placing a solid object---a large glass bottle (filled with water so it sinks)---inside of the tank, which remains stationary;
...
Adopting ``yesterday's posterior is today's prior'' \cite{calvetti2010subjective}, 
our multi-variate Gaussian posterior distribution of the parameter vector $\boldsymbol \Theta$ and noise variance $\Sigma^2$ from (eqn.~\ref{eq:post_theta_sigma}) model calibration now serves as our prior distribution.
...
We adopt the principle of indifference and impose a relatively diffuse prior over the area of the exogenous solid as a function of height, but impose smoothness to counter instability in this inverse problem \cite{groetsch1993inverse}. By imposing a relatively diffuse prior on the solid's shape, we allow the water level time series data to ``speak for itself'' about the solid's shape.
Specifically, we impose a uniform distribution on the area of the bottom base of the solid, $A_0$ (the random variable representing $\alpha(0)$)
\begin{equation}
	A_0 \mid A_b=a_b \sim \mathcal{U}(0, a_b).
\end{equation}
spanning from zero (lack of an exogenous solid) to the area of the bottom base of the tank (largest solid that can fit) $a_b$. 
For the smoothness prior \cite{calvetti2018inverse}, we sequentially correlate the solid's area's at the sequence of heights: 
\begin{equation}
 A_i - A_{i-1} \sim \mathcal{N}(0, \gamma^2) \text{ for } i \in \{1, ..., n\}.
\end{equation} 
The zero mean Gaussian distribution on the difference in the area of the object at two adjacent heights means we expect the function $\alpha(h)$ to be locally flat and change slowly. 
The variance hyperparameter $\gamma^2$ controls the degree to which we promote smoothness in $\alpha(h)$ (smaller $\gamma^2$ gives more smoothness). 

\paragraph{Visualization of the prior.}
Fig.~\ref{fig:prior_area} displays samples of our prior distribution on the solid's area as a function of height---by sampling $\mathbf{A}$ from the prior then constructing $\alpha(h)$ from eqn.~\ref{eq:alpha_basis}.
...
Fig.~\ref{fig:posterior_area} shows posterior samples of the area of the solid as a function of height---the BSI solution to the reconstruction problem.
...
Second, the posterior of the solid's shape agrees reasonably well with the measured area of the object (mean residual: 7.2\,cm$^2$), but some bias is present in $h\in[15, 20]$\,cm.
...
\begin{figure}[h!]
    ...
    \caption{
      ...
      (a) We fill our tank, containing heavy glass bottle, with water, then, at $t=0$, allow it to drain through the orifice in its side.
      (b) Samples of the solid's area as a function of height from the prior, along with the area of the tank (gray) for perspective. 
      ...
      (d) Samples of the solid's (inferred) area as a function of height from the posterior distribution. Gray lines: posterior samples of the area of the tank. Points: held-out, direct measurements of the bottle's area at different heights.
      }
\end{figure}

\section{Conclusions and Discussion}
First, we constructed, Bayesian-calibrated, and tested a forward model for the dynamics of the liquid level in a tank draining of water through a small orifice in its side.
...
The posterior distribution of the bottle's area as a function of height agreed reasonably well with our measurements of its area (mean absolute residual of 7.2\,cm$^2$) but exhibited a little bias. 
...
Our methodology herein could be employed throughout engineering and the applied sciences to infer the shape of an exogenous solid inside of an opaque liquid-holding tank with access to liquid level measurements.

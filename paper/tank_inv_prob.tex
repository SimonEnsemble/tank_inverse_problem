...
\begin{abstract}
In engineering, we may encounter an open-top, liquid-holding tank that is draining via gravity-driven flow through a small orifice in its side. 
...
First, we conduct a tank drainage experiment without an object inside of it and collect liquid level time series data to calibrate and test our forward model for the liquid level over time. Second, we conduct a tank drainage experiment with an object inside of it, then leverage the calibrated forward model and the liquid level time series data to predict the shape of the object inside the tank with quantified uncertainty.
The effectiveness of our approach may be practically useful to infer the shape of an unknown object or porosity of small packed particles (e.g., sand or gravel) inside of an opaque tank with access to liquid level measurements. 
\newline
